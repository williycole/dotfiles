...
local gonvim = {
  ...
  config = function()
    -- require('go').setup {} -- defaults setup, see docs for options
    require('go').setup {
...
local goimplements = {
  -- docs: https://github.com/maxandron/goplements.nvim, Visualize Go struct and interface implementationsfor interface visualizastions
  {
    ...
    ft = { 'go', 'gomod' },
    opts = {
      -- your config goes here esle leave blank
    },
  },
}

-- ⭐ ⭐ ⭐ All lsps setup here autocmd is the recommended setup approach⭐ ⭐ ⭐, docs:  https://github.com/neovim/nvim-lspconfig
-- Automatically install LSPs & related tools to stdpath for Neovim. Add LSP Deps here for anything custom.
...
local lsps = {
  ...
  config = function()
        ...
        if client.supports_method 'textDocument/formatting' then
          vim.api.nvim_create_autocmd('BufWritePre', {
            ...
            callback = function()
              vim.lsp.buf.format {
                filter = function(c)
                  return c.name == 'ts_ls' or c.name == 'angularls'
                end,
                async = false,
              }
            end,
    ...
    -- NOTE: Names here might not match above names. Only use Mason Official LSP names here. For names see :Mason
    ...
    local capabilities = vim.lsp.protocol.make_client_capabilities() -- Extend lsp capabilities as needed
...
return { lsps, gonvim, goimplements }
